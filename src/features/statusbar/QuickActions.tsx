...
export function QuickActions() {
  ...
  return (
    <div className="flex items-center gap-0.5">
      <button type="button" title="New note" className={btnCls} onClick={focusNoteCapture}>
        <NotebookPen size={16} strokeWidth={1.75} />
      </button>
      <button type="button" title="New task" className={btnCls} onClick={() => openTaskEditor()}>
        <Plus size={16} strokeWidth={1.75} />
      </button>
      <button type="button" title="Settings" className={btnCls} onClick={openSettings}>
